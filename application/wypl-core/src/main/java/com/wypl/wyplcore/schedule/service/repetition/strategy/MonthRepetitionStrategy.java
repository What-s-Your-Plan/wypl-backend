...
public class MonthRepetitionStrategy implements RepetitionStrategy {
	...
	public List<ScheduleFindResponse> getScheduleResponses(Schedule schedule, LocalDate searchStartDate,
		...
		// 탐색할 시작 일시와 끝 일시 설정
		LocalDateTime searchEndDateTime = LocalDateTime.of(searchStartDate.withDayOfMonth(endDayOfMonth),
		...
		for (LocalDate date = searchStartDateTime.toLocalDate(); !date.isAfter(searchEndDate); date = date.plusMonths(
			1)) {
